...
# ╔═╡ bfa2f49c-abc6-4af2-b93e-903a87548c6b
let
...
# ╔═╡ 9277a032-af0c-4d7d-b74a-102bce8ad158
f(t, x, y) = y^3
...
# ╔═╡ 0c9c8165-e5d8-4d01-9fbb-ab97c71d6841
md"""
## Transformada amplitud-fase
"""

# ╔═╡ d737d003-b227-4103-a1e1-b6003aa4bf2f
ampfas_model = @ode_def begin
	dr = - ε * sin(t + θ) * f(t, r * cos(t + θ), -r * sin(t + θ))
	dθ = - ε/r * cos(t + θ) * f(t, r * cos(t + θ), -r * sin(t + θ))
end ε

# ╔═╡ 729326c6-7b64-4fc3-a698-4fe0a5128491
md"""
## Promediada
"""
...
# ╔═╡ 936c2bce-71b9-4fb8-9ea3-50eb7d460747
function err(eps, t)
	sol = let
		prob = ODEProblem(ampfas_model, [2.0;0], (0.0, t/eps), [eps])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	sol_prom = let
		prob = ODEProblem(ampfasprom_model, [2.0;0], (0.0, t/eps), [eps])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	return maximum(norm(sol(s)-sol_prom(s)) for s in 0:0.1:t/eps)
end

# ╔═╡ 2945d73f-ca4a-4f22-95a1-e4ecbd88217e
let
	gr(size=(750,400))
	t = 30
	ampfas_sol = let
		prob = ODEProblem(ampfas_model, [2.0;0], (0.0, t), [0.01])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	ampfasprom_sol = let
		prob = ODEProblem(ampfasprom_model, [2.0;0], (0.0, t), [0.01])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	...
	p2 = let
		xlims = (1e-4/2,2*1e-1); ylims = (1e-4/2,2)
		r = (log10(ylims[2]) - log10(ylims[1]))*(xlims[2] - xlims[1])/((log10(xlims[2]) - log10(xlims[1]))*(ylims[2] - ylims[1]))
		values = [1e-4, 1e-3, 1e-2, 1e-1]
		scatter(values, err.(values, 0.3), xaxis=:log, yaxis=:log, xlim=xlims, ylim=ylims, xticks=[1e-4, 1e-3, 1e-2, 1e-1], yticks=[1e-4, 1e-3, 1e-2, 1e-1, 1], ratio=r, xlabel=L"\varepsilon", ylabel="Diferencia", leftmargin = 10mm, legend=false)
