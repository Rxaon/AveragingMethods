...
# ╔═╡ e3e5459a-f074-11ef-1995-15408308f21f
using Plots, DifferentialEquations, ParameterizedFunctions, LaTeXStrings, Measures, LinearAlgebra

# ╔═╡ 07df5322-b489-4137-a7b1-d04b6c6de2f2
let
...
# ╔═╡ 4951342e-42ae-459c-bfa7-4bef35eb2074
ε = 0.1

# ╔═╡ 154dd305-4e64-4a7c-891c-3f5634f92a4e
md"""
## Diagrama de fase
"""

# ╔═╡ f9d727ce-e592-4319-ba1a-77aaedbf7cc6
original_model = @ode_def begin
	dx = y
	dy = -x + ε * (1 - x^2) * y
end ε

# ╔═╡ 8ffcdb9a-b5d3-40f1-b24d-3133c3dca1d5
let
	gr(size=(600,400))
	t = 20
	p = plot(legend=false, ratio=1, lims=(-3,3), xlabel=L"x", ylabel=L"y")
	for x0 = 0.7:1.5:3
		sol = let
			prob = ODEProblem(original_model, [x0; x0], (0.0, t), [ε])
			solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
		end
		plot!(p, sol, idxs=(:x, :y), arrow=true, c=2, l=:dash)
		scatter!(p, [x0], [x0], c=2)
	end
	sol = let
		prob = ODEProblem(original_model, [2; 0.0], (0.0, t), [ε])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	plot!(p, sol, idxs=(:x, :y), arrow=true, c=1, lw=3)
	p
end

# ╔═╡ 67b7a299-9186-4b1b-9ba1-9bf1c6f7a88f
md"""
## Diferencia entre soluciones
"""

# ╔═╡ d737d003-b227-4103-a1e1-b6003aa4bf2f
ampfas_model = @ode_def begin
	dr = ε * sin(t + θ) * (1 - r^2 * cos(t + θ)^2) * r * sin(t + θ)
	dθ = ε * cos(t + θ) * (1 - r^2 * cos(t + θ)^2) * sin(t + θ)
end ε

# ╔═╡ 2e856e3e-40ce-48af-a979-07d02ba37334
ampfasprom_model = @ode_def begin
	dr = ε/2 * r * (1 - r^2/4)
	dθ = 0
end ε

# ╔═╡ fbbb5be5-70fd-4175-8c1a-aef62488b381
function err(eps, t)
	sol = let
		prob = ODEProblem(ampfas_model, [3, 0.0], (0.0, t/eps^1), [eps])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	sol_prom = let
		prob = ODEProblem(ampfasprom_model, [3, 0.0], (0.0, t/eps^1), [eps])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	return maximum(norm(sol(s)-sol_prom(s)) for s in 0:0.1:t/eps^1)
end

# ╔═╡ e3e12ffa-4e9a-451c-8423-f10b7f8dd525
let
	gr(size=(750,400))
	t = 10
	ampfas_sol = let
		prob = ODEProblem(ampfas_model, [3, 0.0], (0.0, t), [0.1])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	ampfasprom_sol = let
		prob = ODEProblem(ampfasprom_model, [3, 0.0], (0.0, t), [0.1])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	p = plot(ampfas_sol, idxs=[1], xlabel=L"t", ylabel="Amplitud", label=L"r")
	plot!(p, ampfasprom_sol, idxs=[1], label=L"\overline{r}")
	q = plot(ampfas_sol, idxs=[2], xlabel=L"t", ylabel="Fase", label=L"\theta")
	plot!(q, ampfasprom_sol, idxs=[2], label=L"\overline{\theta}", leftmargin=3mm)
	p2 = let
		xlims = (1e-4/2,2*1e-1); ylims = (1e-4/2,1/2)
		r = (log10(ylims[2]) - log10(ylims[1]))*(xlims[2] - xlims[1])/((log10(xlims[2]) - log10(xlims[1]))*(ylims[2] - ylims[1]))
		values = [1e-4, 1e-3, 1e-2, 1e-1]
		scatter(values, err.(values, 1), xaxis=:log, yaxis=:log, xlim=xlims, ylim=ylims, xticks=[1e-4, 1e-3, 1e-2, 1e-1], yticks=[1e-4, 1e-3, 1e-2, 1e-1], ratio=r, xlabel=L"\varepsilon", ylabel="Diferencia", leftmargin = 10mm, legend=false)
...
manifest_format = "2.0"
project_hash = "32cfca7c71d6625de3d54ae23b1227009a6a75e1"
