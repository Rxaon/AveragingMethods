...
# ╔═╡ d4c02a96-608e-11f0-2861-e55361b517f5
using DifferentialEquations, Plots, LinearAlgebra, LaTeXStrings, Measures

# ╔═╡ 10a9070f-e4c9-41a7-a063-3fe89930e924
let
...
# ╔═╡ e2b366c3-d591-4d62-ae1a-3b95a6e41ccc
#F(t) = exp(-t^2)
#F(t) = t/(1+t^2)
F(t) = (1+t)^(-0.25)

# ╔═╡ 44f8e054-55fe-42b3-8ae4-e3da1d25d8a4
h(t) = 2-F(t)

# ╔═╡ 7b855ca8-7955-42ae-961d-968609590368
function f(x,p,t)
	p[1]*[
		-x[1]*sin(t+x[2])^2,
		-sin(t+x[2])*cos(t+x[2])
...
# ╔═╡ a915820c-720e-4622-9ccc-c24b691f8348
function err(eps, t)
	sol = let
		prob = ODEProblem(f, [1, 0.0], (0.0, t/eps), [eps])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	return maximum(norm(sol(s)-[exp(-eps*s), 0]) for s in 0:0.1:t/eps)
end

# ╔═╡ 85717a7b-c7dc-4cd4-b4f9-699765494ce7
let
	gr(size=(750,400))
	t = 10
	ampfas_sol = let
		prob = ODEProblem(f, [1, 0.0], (0.0, t), [0.1])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	p = plot(ampfas_sol, idxs=[1], xlabel=L"t", ylabel="Amplitud", label=L"r")
	plot!(p, 0:t, t -> exp(-0.1*t), label=L"\overline{r}")
	q = plot(ampfas_sol, idxs=[2], xlabel=L"t", ylabel="Fase", label=L"\theta", ylim=(-0.12, 0.02))
	plot!(q, 0:t, t -> 0, label=L"\overline{\theta}", leftmargin=3mm)
	p2 = let
		xlims = (1e-4/2,2*1e-1); ylims = (1e-4/2,2e-1)
...
# ╔═╡ 4e96bcee-d9a8-4c12-bb85-3fa975d1f1c8
let
	gr(size=(600,400))
	
	function err(eps, t, f)
		sol = let
			prob = ODEProblem(f, [1, 0.0], (0.0, t/eps), [eps])
			solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
		end
	...
	F1(t) = exp(-t^2); h1(t) = 2-F1(t)
	F2(t) = t/(1+t^2); h2(t) = 2-F2(t)
	F3(t) = (1+t)^(-0.25); h3(t) = 2-F3(t)
	...
	xlims = (1e-4/2,2*1e-1); ylims = (1e-4/2,0.5)
	r = (log10(ylims[2]) - log10(ylims[1]))*(xlims[2] - xlims[1])/((log10(xlims[2]) - log10(xlims[1]))*(ylims[2] - ylims[1]))
	values = [1e-4, 1e-3, 1e-2, 1e-1]
	plot(xaxis=:log, yaxis=:log, xlim=xlims, ylim=ylims, xticks=[1e-4, 1e-3, 1e-2, 1e-1], yticks=[1e-4, 1e-3, 1e-2, 1e-1], ratio=r, xlabel=L"\varepsilon", ylabel="Diferencia", leftmargin = 10mm)
	scatter!(values, err.(values, 1, f1), c=1, label=false)
	plot!(values, values, l=:dash, c=1, alpha=0.5, label=L"\varepsilon")
	scatter!(values, err.(values, 1, f2), c=2, label=false)
	plot!(values, values .* abs.(log.(values)), l=:dash, c=2, alpha=0.5, label=L"\varepsilon\, |\!\!\log(\varepsilon)|")
	scatter!(values, err.(values, 1, f3), c=3, label=false)
	plot!(values, values.^0.25, l=:dash, c=3, alpha=0.5, label=L"\varepsilon^{0.25}")
end
