...
# ╔═╡ 8f393a38-6c66-11f0-110b-e1316692fb0f
using Plots, DifferentialEquations, Measures, LaTeXStrings

# ╔═╡ ca232c8e-c7aa-4b62-978d-601eb1f14c75
let
	using PlotUtils, PlotThemes, Colors
	include("mytheme.jl")
	theme(:mytheme)
end

# ╔═╡ 45a339fd-c4d9-43a5-a54b-4ed4c979aa38
function f(t)
	T = 0
	n = 1
	while t >= T
		...
		if t < T return 0 end
		T += 1
		if t < T return t - floor(t) end
		T += n - 1
		if t < T return 1 end
		T += 1
		if t < T return 1 - (t - floor(t)) end
		n += 1
	end
end

# ╔═╡ 03f99f87-4965-4538-8537-58c43afc4cc7
function err(eps, t0, t)
	function F(dx,x,p,t)
		dx[1] = p[1]*f(t)
	end
	sol = let
		prob = ODEProblem(F, [0.0], (t0, t0 + t/eps), [eps])
		solve(prob, Vern9(), reltol=1e-8, abstol=1e-8)
	end
	return sol(t/eps)[1] + 1e-10
end

# ╔═╡ a949ab28-9ee2-4216-a52c-34adfd411c0a
let
	t0 = 10.0
	T = 10.0
	ε = 0.1
	...
	prob = ODEProblem(F, [0.0], (t0, t0 + T/ε), [ε])
...
manifest_format = "2.0"
project_hash = "8750a2d829b14eb25bbd5a36446ac76b773921a9"
