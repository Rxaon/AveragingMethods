...
# ╔═╡ 2994f63c-5a90-11f0-1ddd-636cf5e3883c
using Plots, LaTeXStrings

# ╔═╡ 66204900-bcad-4dbc-a752-5d13b41094c2
let
...
# ╔═╡ cfbf65b9-e19b-4aa0-af2a-e1012c5b309e
let
	f(x) = (-1)^floor(x)*sqrt((x-floor(x))*(1-x+floor(x)))
	p = plot(xlabel=L"x", ylabel=L"f(t,x)", xlim=(0, 2), legend=false)
	for i in 2:6
		plot!(p, 0:2/2^i:2, f, color=:gray, alpha=exp(-2/i))
	end
	plot!(p, 0:0.001:2, f, color=1)
	scatter!(p, 0:2/2^6:2, f, color=:gray, ms=3)
end
